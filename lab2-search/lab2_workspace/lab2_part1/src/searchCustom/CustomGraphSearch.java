	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		while(!frontier.isEmpty()) {
			...
			if (p.isGoalState(currentState))
			{
				...
				return path; // Return the path from the start to the goal
			}
			...
			for (GridPos childState : childStates) {
				...
				// Check if the child has not been explored or is not already in the frontier
				if(!explored.contains(childNode) && !frontier.contains(childNode)) {
					if(insertFront) {
						frontier.addNodeToFront(childNode); // Stack behavior for DFS (LIFO)
					}
					else
